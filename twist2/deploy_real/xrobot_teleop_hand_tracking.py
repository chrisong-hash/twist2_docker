"""
Teleop with Pico Hand Tracking + Inspire Hands

Full body teleop with ACTUAL hand tracking from Pico VR headset.
Fingers are tracked via Pico and mapped to Inspire robot hands.

Usage:
  conda activate gmr
  python xrobot_teleop_hand_tracking.py --robot unitree_g1 --use_inspire_hands

Controls:
- Right controller key_one: Cycle through idle -> teleop -> pause -> teleop...
- Left controller key_one: Exit program from any state
- Left controller axis_click: Emergency stop
- 👏 Clap: Pause/Resume (safety)
- Body + Hand tracking: Pico VR headset

States:
- idle: Waiting for input or data
- teleop: Full body + hand tracking active
- pause: Robot frozen at current position
...
class XRobotTeleopToRobot:
    ...
    def initialize_all_systems(self):
        ...
        if self.use_inspire_hands:
            print("- Inspire hands: ENABLED (trigger-based control)")
        else:
...
def parse_arguments():
    ...
    parser.add_argument(
        "--use_inspire_hands",
        action="store_true",
        help="Enable Inspire hand control via trigger.",
    )
    parser.add_argument(
        "--inspire_left_ip",
        type=str,
